    ...
    <div>
      ...
      {!hasBrief ? (
        <div className="guide-notice-warning mb-8">
          ...
          <p className="text-xs text-muted-foreground mt-1">브리프 없이도 규칙표를 참고할 수 있지만, 브리프가 있으면 현재 고객사에 해당하는 규칙이 자동 하이라이트됩니다.</p>
        </div>
      ) : (
        /* 브리프 기반 요약 카드 */
        <div className="guide-card-accent mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
            ...
            <div className="bg-background rounded-lg p-3 border border-border/50">
              ...
              <p className="font-semibold text-card-foreground text-xs">{matchedLayout.length > 0 ? matchedLayout.map(l => l.result.split("—")[0].trim()).join(", ") : "기본"}</p>
            </div>
            <div className="bg-background rounded-lg p-3 border border-border/50">
