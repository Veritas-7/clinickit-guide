    ...
    <div className="guide-card mb-4">
      ...
      <div className="w-full bg-secondary rounded-full h-1.5 mb-3">
        <div
          ...
          style={{ width: `${total ? (done / total) * 100 : 0}%`, background: "hsl(var(--accent))" }}
        />
      </div>
      <ul className="space-y-1.5">
        {items.map((item, i) => (
          <li
            key={i}
            className="flex items-start gap-2.5 cursor-pointer py-1 hover:bg-surface rounded px-1 -mx-1 transition-colors"
            onClick={() => toggle(i)}
          >
            {checked.has(i) ? (
            ...
            <span className={`text-sm leading-relaxed ${checked.has(i) ? "line-through text-muted-foreground" : "text-foreground/85"}`}>
              {item}
            </span>
          </li>
        ))}
  ...
  return (
    <div>
      ...
      <div className="guide-notice-info mb-8">
        <p className="text-sm">각 항목을 클릭하면 체크/해제할 수 있습니다. 이 체크 상태는 페이지를 벗어나면 초기화됩니다.</p>
      </div>
